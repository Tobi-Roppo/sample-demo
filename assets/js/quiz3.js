const quizData = [
  ...
  function printQuizResults() {
    ...
    printWindow.document.write('.correct-answer { color: green; }');
    printWindow.document.write('</style>');
    ...
    // Include user's name in the printed results
    ...
    : "Type your answer";
      const isCorrect = Array.isArray(correctAnswer) ? correctAnswer.includes(userAnswer.toLowerCase()) : userAnswer.toLowerCase() === correctAnswer.toLowerCase();
      const answerClass = isCorrect ? 'correct-answer' : '';
      const correctAnswerHTML = Array.isArray(correctAnswer) ? `<br><strong class="${isCorrect ? 'correct-answer' : 'incorrect-answer'}">Correct Answer:</strong> ${correctAnswer.map((ans) => `<li>${ans}</li>`).join("")}</ul>` : `<br><span class="${answerClass}"><strong>Correct Answer:</strong> ${correctAnswer}</span>`;

      printWindow.document.write(`<p><strong>${index + 1}. ${question}</strong><br/>${options} Your Answer: ${userAnswer} ${correctAnswerHTML}</p>`);
    });
